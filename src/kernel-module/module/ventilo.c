...
MODULE_DESCRIPTION("Dreamyfan Module, v0.2");
MODULE_LICENSE("GPL");
...
static void pbm_to_usbdata(uint8_t id, char *raster, uint16_t *display)
{
	uint8_t col_end;
	uint8_t i;
	uint8_t j;
...
static u64 set_config(uint8_t id,
		uint8_t open_opt,
		uint8_t close_opt,
		uint8_t turn_opt)
{
...
{
	int ret = usb_register(&ventilo_driver);
